function [W, H] = matchCol(W,Wref,H)
% column normalization
W2 = col_norm(W);
Wref = col_norm(Wref);

% cost matrix : negative correlations between the columns
P_cost = -abs(Wref'*W2);

% Permutation indexes given by the Hungarian algorithm
[matchId,~]=munkres1(P_cost);
 matchId(find(matchId==0))=[];
...
W = W(:,matchId);
 ...
 if nargin == 3
     % on permute H en conséquence si jamais H est fourni aussi
   H = H(matchId,:);  
 elseif nargin == 2
   H = [];
 end
...
function [assignment,cost] = munkres1(costMat)
